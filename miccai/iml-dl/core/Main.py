...
class Main(object):
    ...
    def setup_experiment(self):
        ...
        logging.info("[Main::setup_experiment]: ################ Starting "
                     "setup ################")
        now = datetime.now()
        ...
        # save config file under trainer checkpoint path:
        if not os.path.exists(self.config_file['trainer']['params']['checkpoint_path']):
            os.makedirs(config_file['trainer']['params']['checkpoint_path'])
        with open(config_file['trainer']['params']['checkpoint_path']
                  + "/config.yaml", "w") as file:
            yaml.dump(config_file, file)

        # Initialize Configurator
        if self.config_file['configurator'] is None:
            logging.info("[Main::setup_experiment::ERROR]: Configurator module"
                         " not found, exiting...")
            exit()
        ...
        configurator = configurator_class(config_file=self.config_file,
                                          log_wandb=True)
        ...
        wandb.init(project=exp_name, name=method_name, config=config_dict,
                   id=date_time)
